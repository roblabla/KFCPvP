...
public class ArenaTimer {
	...
	public static void run() {
        ...
        for(Arena arena : KFCPvP.getInstance().getArenaManager().getArenas()) {
        	if(!arena.getPlayers().isEmpty()){
	            int seconds = arena.getSeconds();
	            if(arena.getGameStatus() == ArenaStatus.inGame) {
	            	for(String name : arena.getPlayers())
	            		KFCPvP.getInstance().getStatManager().getPlayerStats(name).getStat(StatType.PlayingTime).addAmount(1);
	            	if(seconds == 60)
